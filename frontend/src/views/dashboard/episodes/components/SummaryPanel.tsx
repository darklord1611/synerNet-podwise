...
export default function SummaryPanel(props: { summary: any, onSeek?: (time: number) => void }) {
	// Chakra Color Mode
    const { onSeek, summary } = props;
			...
			<VStack spacing='25px' alignItems='start' w='2xl' m='auto'>
                ...
                <VStack spacing='10px' w='full' alignItems='start'>
                    ...
                    <UnorderedList p={3} bg={boxBg} borderRadius='10px'>
                        {summary.takeaways.map((takeaway: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal) => {
                            return (
                                <ListItem 
                                    ml={3}
                                    fontWeight={400} 
