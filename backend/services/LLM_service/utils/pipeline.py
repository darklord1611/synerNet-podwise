...
class LLMPipeline:
  ...
  def _set_optimization_instruction(self, article, initial_summary, response_feedback):
    prompt = self.set_prompt(article, task = "summary")
    self.evaluation_instruction = f"""
        Here's the prompt to summarize: {prompt}. 
        The intial summary generated by the above prompt is: 
        ${initial_summary}
  ...
  def summarize(self, article, optimize=False):
    ...
    if optimize:
      final_summary, response_feedback = self._optimize_summary(article, initial_summary)
      optimized_prompt, feedback = self._optimize_prompt(article, initial_summary, response_feedback)
    else:
  ...
  def _optimize_summary(self, original_input, initial_summary):
    input_summary = tg.Variable(initial_summary,
                       ...
                       requires_grad=True)
    self._set_optimization_instruction(original_input, initial_summary, response_feedback=None)
    ...
    print(vars(input_summary))
    print(input_summary.get_gradient_text(), "Nothing here")
    
    print(loss.value)
    
    with open("demo/temp_response_gradient_feedback.txt") as f:
      response_feedback = f.read()
    
    print("Optimize the prompt right now")
    optimized_prompt, feedback = self._optimize_prompt(original_input, input_summary.value, response_feedback)
    
    print(optimized_prompt)
    return input_summary.value, loss.value
